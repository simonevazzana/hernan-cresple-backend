...
const { dribbleStartQuery } = require('../utils/db-query-utils')
...
const dribbleStartHandler = async (request, reply) => {
  ...
  while (!endPlayer) {
    startPlayer = (await Players.aggregate([
      { $match: { active: true, ...dribbleStartQuery({ difficulty, league, season }) } },
      { $sample: { size: 1 } }
    ]).toArray())[0]
        ...
        $match:
            {
              ...
              active: true,
              ...dribbleStartQuery({ difficulty, league, season }),
              $and: [
                {
