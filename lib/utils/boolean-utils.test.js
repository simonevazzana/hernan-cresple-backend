...
const { checkSeason, checkTeamPosition, checkLeague } = require('./boolean-utils')

test('checkLeague() should correctly return true, if there\'s no league in input', t => {
  const team = {
    active: true,
    league: 'bundesliga',
  ...
  const res = checkLeague({ team })
  t.is(res, true)
})

test('checkLeague() should correctly check that a team played in a certain league, using the normalized text', t => {
  const team1 = {
    active: true,
    league: 'bundesliga',
    name: 'Bayern Munich 2022/2023',
  ...
  const league = 'BuNdEsLiG A'

  const res1 = checkLeague({ team: team1, league })
  t.is(res1, true)
  ...
  const res2 = checkLeague({ team: team2, league })
  t.is(res2, false)
})
